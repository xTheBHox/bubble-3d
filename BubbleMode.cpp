...
void BubbleMode::update(float elapsed) {
  ...
  // 1. Update player velocity
  {
    ...
    player_dvel = glm::vec3(ca, sa, 0.0f) * player_dvel.x + glm::vec3(-sa, ca, 0.0f) * player_dvel.y;

    level.player.vel += player_dvel;
    level.player.vel *= std::pow(0.5f, elapsed / 0.1f); // friction
  ...
  // 4. Update bullet-ball collisions

  // 5. Update bullet-wall collisions

  // 6. Update ball-wall collisions

  // 7. Update ball-ball collisions

  // 8. Update ball-player collisions
  ...
  // 10. Update ball positions

  // 11. Update bullet positions
